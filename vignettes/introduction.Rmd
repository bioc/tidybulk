...
`plot_reduced_dimension()` | Plot reduced dimensions (PCA includes % variance)
...
airway = airway[rowVars(assay(airway, "counts_tmm")) > 0, ]
```
...
airway = airway |>
  reduce_dimensions(assay = "counts_tmm", method="MDS", .dims = 2)
```

```{r eda-pca-analysis, fig.width = 10, fig.height = 10}
airway = airway |>
  reduce_dimensions(assay = "counts_tmm", method="PCA", .dims = 2)
```
...
Alternatively, `plot_reduced_dimension()` builds the same plots from a `SummarizedExperiment`. For PCA it also annotates each axis with the percentage of variance explained.

```{r eda-plot-reduced-dimension, fig.width = 10, fig.height = 10}
airway |>
  plot_reduced_dimension(.color = dex, method = "MDS", dims = 1:2) +
  my_theme +
  labs(title = "MDS Analysis")

airway |>
  plot_reduced_dimension(.color = dex, method = "PCA", dims = 1:2) +
  my_theme +
  labs(title = "PCA Analysis")
```
...
glmmSeq [@ma2020glmmseq] is used for mixed models. Pass `formula_dispersion` as a fixed-effects formula for edgeR tagwise dispersion (you choose whether grouping factors enter that design). Omit it, or pass `.dispersion` for a precomputed column; with many samples, omitting both lets each gene estimate phi.
...
airway |>
  ...
  # Fit model; formula_dispersion is the edgeR design for tagwise phi
  test_differential_expression(
    ~ dex + (1|cell), 
    method = "glmmseq_lme4",
