...
dgList <- normLibSizes(dgList, method="TMM")
...
We may want to reduce the dimensions of our data, for example using PCA or MDS algorithms. `reduce_dimensions` takes a `SummarizedExperiment`, the assay name via the character argument `assay`, and a method (e.g., MDS or PCA), then adds the reduced dimensions to the object metadata.
...
Alternatively, `plot_reduced_dimension()` can plot the reduced dimensions directly. For PCA it also reports the percentage of variance explained on each axis.

```{r plot_reduced_dimension_pca, fig.width = 10, fig.height = 10}
airway.norm.PCA |>
